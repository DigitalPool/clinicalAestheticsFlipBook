...
function setPreview(view) {
	...
	// In the flipbook's JavaScript file (inside the iframe)
function initTurnJSBookResize() {
  // Initialize turn.js
  $('#book').turn({
    width: 1440,
    height: 900,
  ...
  // Function to resize the book
  function resizeBook(newWidth, newHeight) {
    $('#book').turn('size', newWidth, newHeight);
  }
  
  // Listen for resize messages from parent window
  window.addEventListener('message', function(event) {
    if (event.data.type === 'resizeTurnJS') {
      resizeBook(event.data.width, event.data.height);
    }
  });
  
  // Also handle window resize inside iframe
  let resizeTimer;
...
$(document).ready(initTurnJSBookResize);
}
